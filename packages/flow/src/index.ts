...
@customElement('gem-bind-flow-canvas')
@adoptedStyle(canvasStyle)
export class GemBindFlowCanvasElement extends DuoyunResizeBaseElement {
  ...
  #renderNode = (data: any, node: Node) => {
    return this.renderNode
      ...
      : html`<div class="node" part=${GemBindFlowElement.node}>${data}</div>`;
  };

  #renderNodeLabel = (label: string | undefined, edge: Edge) => {
    return this.renderNodeLabel
      ? this.renderNodeLabel(label, edge)
      : html`<div class="node-label" part=${GemBindFlowElement.nodeLabel}>${label}</div>`;
  };
  ...
  #renderEdgeLabel = (label: string | undefined, edge: Edge) => {
    return this.renderEdgeLabel
      ...
      : html`<div class="edge-label" part=${GemBindFlowElement.edgeLabel}>${label}</div>`;
  };
...
@customElement('gem-bind-flow')
@adoptedStyle(style)
@shadow()
export class GemBindFlowElement extends DuoyunResizeBaseElement {
  ...
  #canvasRef = createRef<GemBindFlowCanvasElement>();

  #state = createState<State>({});

  #setScale = ({ width }: GemBindFlowCanvasElement['contentRect']) => {
  ...
  #onCanvasResize = (evt: CustomEvent) => {
    const { contentRect } = evt.target as GemBindFlowCanvasElement;
